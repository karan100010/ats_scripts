...
class LanguagePredictionUser(HttpUser):
    ...
    def on_start(self):
        ...
        self.filepath = random.choice(self.audio_files)
    ...
    def predict_language(self):
        ...
        self.audio_length = get_audio_length_from_url(self.filepath)       
        print(f"Audio length: {self.audio_length:.4f} seconds")
    
        with self.client.post("/predict_language", json=data, headers=headers, catch_response=True) as response:
